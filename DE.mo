package DE
  ...
  model EN253pipe
    // ---------- Definitions ----------
    // ---------- Inheritances ----------
    extends Modelica.Fluid.Pipes.DynamicPipe(redeclare replaceable package Medium = Modelica.Media.Water.StandardWater constrainedby 
    Modelica.Media.Interfaces.PartialMedium, redeclare replaceable model HeatTransfer = Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.IdealFlowHeatTransfer 
    constrainedby Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.PartialFlowHeatTransfer, final heatTransfer.k = lambda_insul * 2 * Modelica.Constants.pi / 
    Modelica.Math.log(outsideDiaInsul / 2 / (outsideDiaPipe / 2)), final diameter = insideDiaPipe, final use_HeatTransfer = true 
...
// ---------- Graphics & Layout ----------
  ...
  // ---------- Inheritances ----------
  // ---------- Declarations ----------
    parameter Integer HExSections=100;
    parameter Real HExAlpha=4000;
    parameter Real HExWallConductivity=20;
    parameter Real HExWallThick=0.0005;
    replaceable package distrMedium = Modelica.Media.Water.StandardWaterOnePhase constrainedby Modelica.Media.Interfaces.PartialMedium;
  ...
  // ---------- Components ----------
    Modelica.Fluid.Pipes.DynamicPipe distrPipe(
  	redeclare replaceable package Medium = distrMedium, 
  	redeclare replaceable model FlowModel = Modelica.Fluid.Pipes.BaseClasses.FlowModels.DetailedPipeFlow, 
  	 redeclare replaceable model HeatTransfer = Modelica.Fluid.Pipes.BaseClasses.HeatTransfer.ConstantFlowHeatTransfer(alpha0 = HExAlpha), 
  	 length = 10, 
  	 diameter = 0.01, 
  	 nParallel = HExSections, 
  	 use_HeatTransfer = true) 
  	 annotation(Placement(visible = true, transformation(origin = {-30, -20}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
    
    Modelica.Fluid.Pipes.DynamicPipe bldgPipe(
    ...
    Modelica.Thermal.HeatTransfer.Components.ThermalConductor HExMaterial(G = HExWallConductivity * HExWallThick) 
  	annotation(Placement(visible = true, transformation(origin = {0, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  	
    replaceable Modelica.Fluid.Valves.ValveIncompressible valve(
  	redeclare replaceable package Medium = distrMedium, 
  	dp(displayUnit = "Pa"), 
  	dp_nominal(displayUnit = "Pa"), 
  	CvData = Modelica.Fluid.Types.CvTypes.OpPoint, 
  	allowFlowReversal = true, show_T = true) 
  	annotation(Placement(visible = true, transformation(extent = {{-50, -80}, {-70, -60}}, rotation = 0)));
  	
    replaceable Modelica.Fluid.Machines.ControlledPump bldgPump(
  	redeclare replaceable package Medium = bldgMedium, 
  	checkValve = true) 
  	annotation(Placement(visible = true, transformation(origin = {60, -70}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  	
     replaceable Modelica.Fluid.Interfaces.FluidPort_a distrHWS(
  	redeclare replaceable package Medium = distrMedium) 
  	annotation(Placement(visible = true, transformation(origin = {-90, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  	
    replaceable Modelica.Fluid.Interfaces.FluidPort_b distrHWR(
  	redeclare replaceable package Medium = distrMedium) 
  	annotation(Placement(visible = true, transformation(origin = {-90, -70}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  	
    replaceable Modelica.Fluid.Interfaces.FluidPort_a bldgHWR(
  	redeclare replaceable package Medium = bldgMedium) 
  	annotation(Placement(visible = true, transformation(origin = {90, -70}, extent = {{10, -10}, {-10, 10}}, rotation = 0), iconTransformation(extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  	
    replaceable Modelica.Fluid.Interfaces.FluidPort_b bldgHWS(
  	redeclare replaceable package Medium = bldgMedium) 
  	annotation(Placement(visible = true, transformation(origin = {90, 30}, extent = {{10, -10}, {-10, 10}}, rotation = 0), iconTransformation(extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  replaceable Modelica.Blocks.Interfaces.RealInput u_s annotation(Placement(visible = true, transformation(origin = {90, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 180), iconTransformation(origin = {90, 82}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));  
    
    Modelica.Fluid.Sensors.Temperature bldgTT(
  	redeclare replaceable package Medium = bldgMedium) 
  	annotation(Placement(visible = true, transformation(origin = {60, 50}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  ...
  equation
    connect(bldgTT.T, TC_PID.u_m) annotation(Line(points = {{54, 50}, {0, 50}, {0, 58}}, color = {0, 0, 127}));
    connect(u_s, TC_PID.u_s) annotation(Line(points = {{90, 70}, {12, 70}}, color = {0, 0, 127}));
    connect(TC_PID.y, valve.opening) annotation(Line(points = {{-11, 70}, {-11, 70.5}, {-60, 70.5}, {-60, -62}}, color = {0, 0, 127}));
// ---------- Connectors ----------
    connect(bldgHWR, bldgPump.port_a) annotation(Line(points = {{90, -70}, {70, -70}}));
    connect(bldgPump.port_b, bldgPipe.port_a) annotation(Line(points = {{50, -70}, {30, -70}, {30, -30}}, color = {0, 127, 255}));
    connect(bldgPipe.port_b, bldgTT.port) annotation(Line(points = {{30, -10}, {30, 30}, {60, 30}, {60, 40}}, color = {0, 127, 255}));
    connect(HExMaterial.port_b, bldgPipe.heatPorts[1]) annotation(Line(points = {{10, -20}, {26, -20}}, color = {191, 0, 0}));
    connect(distrPipe.heatPorts[1], HExMaterial.port_a) annotation(Line(points = {{-25.6, -20.1}, {-9.6, -20.1}}, color = {127, 0, 0}));
    connect(distrPipe.port_b, valve.port_a) annotation(Line(points = {{-30, -30}, {-30, -70}, {-50, -70}}, color = {0, 127, 255}));
    connect(distrHWS, distrPipe.port_a) annotation(Line(points = {{-90, 30}, {-30, 30}, {-30, -10}}));
    connect(valve.port_b, distrHWR) annotation(Line(points = {{-70, -70}, {-90, -70}}, color = {0, 127, 255}));
    connect(bldgTT.port, bldgHWS) annotation(Line(points = {{60, 40}, {60, 30}, {90, 30}, {90, 30}}, color = {0, 127, 255}));
// ---------- Equations ----------
// ---------- Graphics & Layout ----------
    annotation(Icon(coordinateSystem(grid = {2, 8}, initialScale = 0.1), graphics = {Rectangle(origin = {-19, 16}, fillColor = {0, 85, 255}, fillPattern = FillPattern.Sphere, lineThickness = 2, extent = {{-40, 40}, {40, -40}}), Line(origin = {5.27, 16}, points = {{74.7293, 32}, {-55.2707, 32}, {6.72932, 0}, {-55.2707, -32}, {74.7293, -32}}, thickness = 2), Line(origin = {-24.27, -52}, points = {{-3.72703, 12}, {12.273, 12}, {-3.72703, -12}, {12.273, -12}, {-3.72703, 12}}, thickness = 2), Line(origin = {-20, -32}, points = {{0, 8}, {0, -8}}, thickness = 2), Line(origin = {-60, -72}, points = {{40, 8}, {40, -8}, {-20, -8}}, thickness = 2, arrow = {Arrow.None, Arrow.Filled}, arrowSize = 10), Line(origin = {-60, 68}, points = {{40, -12}, {40, 12}, {-20, 12}}, thickness = 2, arrow = {Arrow.Filled, Arrow.None}, arrowSize = 10), Ellipse(origin = {60, 80}, lineThickness = 2, extent = {{-15, 15}, {15, -15}}, endAngle = 360), Line(origin = {60, 52}, points = {{0, 12}, {0, -4}}, thickness = 2), Text(origin = {60, 80}, lineThickness = 2, extent = {{-10, 10}, {10, -10}}, textString = "TC", fontName = "DejaVu Sans Mono"), Ellipse(origin = {50, -16}, lineThickness = 2, extent = {{-15, 15}, {15, -15}}, endAngle = 360), Line(origin = {45.33, -16}, points = {{4.67075, -15}, {-9.32925, 0}, {4.67075, 15}}, thickness = 2), Text(origin = {0, 125}, lineColor = {0, 0, 255}, extent = {{-100, 20}, {100, -20}}, textString = "%name", fontName = "DejaVu Sans Mono")}), uses(Modelica(version = "3.2.1")), Diagram);
  end ETS;
